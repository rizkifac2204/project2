...
const handleSubmit = (values, setSubmitting, refetch) => {
  axios
    .put(`/api/caleg/${values.id}`, values)
    .then((res) => {
...
function CalegFormEdit({ caleg, refetch }) {
  const initialvalues = caleg
    ? {
        id: caleg.id,
        jenis: caleg.jenis || "",
        partai_id: "1",
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { setSubmitting }) =>
      handleSubmit(values, setSubmitting, refetch),
  });
  ...
  useEffect(() => {
    if (!jenis) return;
    formik.setFieldValue("partai_id", caleg?.partai_id || "");
    formik.setFieldValue("dapil", caleg?.dapil || "");
