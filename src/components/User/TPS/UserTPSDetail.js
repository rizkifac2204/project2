  ...
  function handleDelete() {
    ...
    if (ask) {
      axios
        ...
        .then((res) => {
          queryClient.invalidateQueries({ queryKey: ["users-tps"] });
          toast.success(res.data.message);
          setTimeout(() => {
            router.push(`/admin/pungut-hitung/user-tps`);
          }, 500);
        })
        .catch((err) => {
          console.log(err);
        ...
        <Box className="mod-profile-detail">
          <Box>
            ...
            <Box>
              ...
              >
                @{user?.kelurahan || "-"} TPS {user?.tps_no || "-"}
              </Typography>
          ...
          {user.editable && !user.myself && (
            <Box className="user-detail--btn text-right">
              <Button
                ...
                sx={{ mr: 1 }}
                component={Link}
                href={`/admin/pungut-hitung/user-tps/${user.id}/edit`}
              >
                Edit
              </Button>
              <Button
