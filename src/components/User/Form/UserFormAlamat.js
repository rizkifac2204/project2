...
const useHandleSubmitMutation = (user) => {
  return useMutation(async (formPayload) => {
    try {
      const res = await axios.put(`/api/user/${user.id}/alamat`, formPayload);
      return res.data;
    } catch (err) {
...
function UserFormAlamat({ user }) {
  ...
  const {
    ...
    queryFn: ({ signal }) =>
      axios
        .get(`/api/user/${user.id}/alamat`, { signal })
        .then((res) => res.data)
        .catch((err) => {
